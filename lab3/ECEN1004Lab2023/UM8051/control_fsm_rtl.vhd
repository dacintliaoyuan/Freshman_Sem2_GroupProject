...
architecture rtl of control_fsm is
  ...
  signal s_tsk : std_logic_vector(7 downto 0);   -- TASK-Flag
...
-- inputs:  state,s_help,s_ti,s_ri,s_ie0,s_ie1, s_tsk, s_tf0,s_tf1,
...
------------------------------------------------------------------------------
 ...
 p_state: process (state,s_help,s_ti,s_ri,s_ie0,s_ie1,s_tsk,s_tf0,s_tf1,
    ...
    if state=STARTUP then
      ...
      if intblock_i = '0' and
        ...
        if state=FETCH then
          s_intpre2_en <= '1';
          s_intpre2_d <= '1';
          s_regs_wr_en <= "0001";                -- increment stackpointer
          s_nextstate <= EXEC1;
        elsif state=EXEC1 then
          if (s_tsk(0) = '1') then   				-- Task 0
            s_help_en <= "01011";                -- interruptvector 002Bh
            s_inthigh_d <= '1';
            s_inthigh_en <= '1';
            s_adr_mux <= "10000";
            s_regs_wr_en <= "0110";              -- reset taskr(0)
            s_ext0isrh_d  <= '1';
            s_ext0isrh_en <= '1';
          elsif (s_tsk(1) = '1') then   			-- Task 1
            s_help_en <= "01100";                -- interruptvector 0033h
            s_inthigh_d <= '1';
            s_inthigh_en <= '1';
            s_adr_mux <= "10001";
            s_regs_wr_en <= "0110";              -- reset taskr(1)
            s_ext0isrh_d  <= '1';
            s_ext0isrh_en <= '1';
          elsif (s_tsk(2) = '1') then   			-- Task 2
            s_help_en <= "01101";                -- interruptvector 003Bh
            s_inthigh_d <= '1';
            s_inthigh_en <= '1';
            s_adr_mux <= "10010";
            s_regs_wr_en <= "0110";              -- reset taskr(2)
            s_ext0isrh_d  <= '1';
            s_ext0isrh_en <= '1';
          elsif (s_tsk(3) = '1') then   			-- Task 2
            s_help_en <= "01110";                -- interruptvector 0043h
            s_inthigh_d <= '1';
            s_inthigh_en <= '1';
            s_adr_mux <= "10011";
            s_regs_wr_en <= "0110";              -- reset taskr(2)
            s_ext0isrh_d  <= '1';
            s_ext0isrh_en <= '1';
          elsif (s_tsk(4) = '1') then   			-- Task 2
            s_help_en <= "01111";                -- interruptvector 004Bh
            s_inthigh_d <= '1';
            s_inthigh_en <= '1';
            s_adr_mux <= "10100";
            s_regs_wr_en <= "0110";              -- reset taskr(2)
            ...
            s_help_en <= "00101";          -- interruptvector 0003h
            s_intlow_d <= '1';
            s_intlow_en <= '1';
            s_adr_mux <= "00001";
            s_regs_wr_en <= "0110";         -- reset IE0
            s_ext0isr_d   <= '1';
            s_ext0isr_en  <= '1';
          elsif (ET0 and s_tf0)='1' then   -- timer interrupt 0 low priority
            s_help_en <= "00110";           -- interruptvector 000Bh
            s_intlow_d <= '1';
            s_intlow_en <= '1';
            s_adr_mux <= "00010";
            s_regs_wr_en <= "0110";         -- reset TF0
          elsif (EX1 and s_ie1)='1' then   -- external interrupt 1 low priority
            s_help_en <= "00111";           -- interruptvector 0013h
            s_intlow_d <= '1';
            s_intlow_en <= '1';
            s_adr_mux <= "00011";
            s_regs_wr_en <= "0110";        -- reset IE1
            s_ext1isr_d   <= '1';
            s_ext1isr_en  <= '1';
          elsif (ET1 and s_tf1)='1' then   -- timer interrupt 1 low priority
            s_help_en <= "01000";           -- interruptvector 001Bh
            s_intlow_d <= '1';
            s_intlow_en <= '1';
            s_adr_mux <= "00100";
            s_regs_wr_en <= "0110";               -- reset TF1
          elsif (ES and (s_ri or s_ti))='1' then -- serial int 0 low priority
            s_help_en <= "01001";                 -- interruptvector 0023h
            s_intlow_d <= '1';
            s_intlow_en <= '1';
          else
            NULL;
          ...
          --s_regs_wr_en <= "0101";        -- write one byte and increment SP
          s_regs_wr_en <= "1001";        -- write one byte and increment SP
          s_nextstate <= EXEC3;
...
--          s_regs_wr_en <= "0100";        -- write one byte
          s_regs_wr_en <= "1000";        -- write one byte
       ...
       -- end of starting interrupt procedure
        ...
        case s_instr_category is
          ...
          when IC_ACALL =>              -- ACALL addr11
            if state=FETCH then
              s_adr_mux <= "01111";      -- adress = sp + 1
              s_data_mux <= "1110";     -- data = (pc+2)(7 downto 0)
              s_regs_wr_en <= "1001";     -- write one byte to RAM and increment SP
          ...
          when IC_ADD_A_D =>            -- ADD A, direct
            if state=FETCH then
              ...
              s_adr_mux <= "01000";      -- adress = rom_data_i
              s_nextstate <= EXEC2;
            elsif state=EXEC2 then
              alu_cmd_o <= ADD_ACC_RAM; -- addition command (ACC + RAM_DATA)
              s_data_mux <= "0011";     -- data = aludata_i
          ...
          when IC_ADD_A_ATRI =>         -- ADD A,@Ri
            if state=FETCH then
              s_adr_mux <= "00111";      -- address = Ri-register
              s_nextstate <= EXEC1;
            elsif state=EXEC1 then
              alu_cmd_o <= ADD_ACC_RAM; -- addition command (ACC + RAM_DATA)
              s_data_mux <= "0011";     -- data = aludata_i
          ...
          when IC_ANL_A_D =>            -- ANL A, direct
            if state=FETCH then
              ...
              s_adr_mux <= "01000";      -- address = rom_data_i
              s_nextstate <= EXEC2;
            elsif state=EXEC2 then
              alu_cmd_o <= AND_ACC_RAM; -- AND command (ACC ^ RAM_DATA)
              s_data_mux <= "0011";     -- data = aludata_i
          ...
          when IC_ANL_D_DATA =>         -- ANL direct, DATA
            if state=FETCH then
              ...
              s_help_en <= "00001";      -- help = rom_data_i
              s_adr_mux <= "01000";      -- adress = rom_data_i
              s_pc_inc_en <= "0001";    -- increment program-counter
              s_nextstate <= EXEC2;
            elsif state=EXEC2 then
              alu_cmd_o <= AND_RAM_ROM;  -- AND command (ROM_DATA_I ^ RAM_DATA)
              s_adr_mux <= "01010";       -- adress = help
              s_data_mux <= "0011";      -- data = aludata_i
              s_regs_wr_en <= "0100";     -- write one byte
              s_pc_inc_en <= "0001";     -- increment program-counter
              s_nextstate <= FETCH;
          ...
          when IC_ANL_C_BIT =>          -- ANL C, bit
            if state=FETCH then
              ...
              s_regs_wr_en <= "0110";    -- write one bit (automatic CY-address)
              s_pc_inc_en <= "0001";    -- increment program-counter
              s_nextstate <= FETCH;
            end if;
          ...
          when IC_CJNE_A_D =>           -- CJNE A, direct, rel
            if state=FETCH then
              ...
              if unsigned(aludata_i) /= 0 then
                s_adr_mux <= "01011";      -- adress of CY
                s_bdata_mux <= "0011";    -- bdata = cby_i
                s_regs_wr_en <= "0110";    -- write one bit
                s_pc_inc_en <= "0010";    -- add relativ adress to PC
              else
                     s_adr_mux <= "01011";        -- adress of CY      (CV: changed V1.5 according to specification: clear carry)
          ...
          when IC_CJNE_RR_DATA =>        -- CJNE RR, #data, rel
            if state=FETCH then
              s_adr_mux <= "00110";       -- address = rr-address
              s_pc_inc_en <= "0001";     -- increment program-counter
              s_nextstate <= EXEC1;
            elsif state=EXEC1 then
              alu_cmd_o <= COMP_ROM_RAM;-- Compare ROM_DATA_I/RAM_DATA operat.
              s_help_en <= "00010";      -- save aludata_i
              s_helpb_en <= '1';        -- save new_cy_i
              s_pc_inc_en <= "0001";    -- increment program-counter
              s_nextstate <= EXEC2;
            elsif state=EXEC2 then
          ...
          when IC_DA_A =>             -- DA A
            ...
            s_data_mux <= "0011";     -- data = aludata_i
            s_regs_wr_en <= "0111";    -- write ACC and CY (special operation)
            s_pc_inc_en <= "0001";    -- increment program-counter
            s_nextstate <= FETCH;
          ...
          when IC_DEC_ATRI =>           -- DEC @Ri
            if state=FETCH then
              ...
              alu_cmd_o <= DEC_RAM;     -- decrement operation
              s_adr_mux <= "00111";      -- address = ri-address
              s_data_mux <= "0011";     -- data = aludata_i
              s_regs_wr_en <= "1000";     -- write one byte to RAM
              s_pc_inc_en <= "0001";    -- increment program-counter
              s_nextstate <= FETCH;
            end if;
          ...
          when IC_DIV_AB =>             -- DIV AB
            if state=FETCH then
              s_adr_mux <= "01100";      -- adress of B register
              ...
              s_data_mux <= "0011";     -- data = aludata_i
              s_adr_mux <= "01100";      -- adress of B register
              s_regs_wr_en <= "0111";    -- write ACC,B,OV,CY(special operation)
              alu_cmd_o <= DIV_ACC_RAM; -- divison operation
              s_pc_inc_en <= "0001";    -- increment program-counter
              s_nextstate <= FETCH;
            end if;
          ...
          when IC_DJNZ_RR =>            -- DJNZ Rr, rel
            if state=FETCH then
              s_adr_mux <= "00110";      -- address = rr-address
              s_help_en <= "00100";      -- save Rr-adress
              s_pc_inc_en <= "0001";    -- increment program-counter
              s_nextstate <= EXEC1;
            elsif state=EXEC1 then
              alu_cmd_o <= DEC_RAM;     -- decrement operation
              if unsigned(aludata_i) /= 0 then
                s_adr_mux <= "01010";    -- address = rr-address
                s_data_mux <= "0011";   -- data = aludata_i
                s_regs_wr_en <= "0100";  -- write one byte
                s_pc_inc_en <= "0010";  -- add relativ adress to PC
              else
          ...
          when IC_DJNZ_D =>             -- DJNZ direct, rel
            if state=FETCH then
              ...
              s_help_en <= "00001";      -- save address
              s_adr_mux <= "01000";      -- address = rom_data
              s_pc_inc_en <= "0001";    -- increment program-counter
              s_nextstate <= EXEC2;
            elsif state=EXEC2 then
              alu_cmd_o <= DEC_RAM;            -- decrement operation
              if unsigned(aludata_i) /= 0 then
                s_adr_mux <= "01010";           -- address = help
                s_data_mux <= "0011";          -- data = aludata_i
                s_regs_wr_en <= "0100";         -- write one byte
                s_pc_inc_en <= "0010";         -- add relativ adress to PC
              else
          ...
          when IC_INC_DPTR =>           -- INC DPTR
            if state=FETCH then
              s_adr_mux <= "01101";      -- adress of DPL
              s_nextstate <= EXEC1;
            elsif state=EXEC1 then
              alu_cmd_o <= INC_RAM;     -- increment operation
              s_help_en <= "00010";      -- help = aludata_i
              s_adr_mux <= "01101";      -- adress of DPL
              s_data_mux <= "0011";     -- data = aludata_i
              s_regs_wr_en <= "0100";    -- write one byte
              s_nextstate <= EXEC2;
            elsif state=EXEC2 then
              s_adr_mux <= "01110";      -- adress of DPH
              s_nextstate <= EXEC3;
            elsif state=EXEC3 then
          ...
          when IC_JB =>                 -- JB bit, rel
            if state=FETCH then
              ...
              s_adr_mux <= "01000";       -- bit adress = rom_data_i
              s_pc_inc_en <= "0001";    -- increment program-counter
              s_nextstate <= EXEC2;
            elsif state=EXEC2 then
          ...
          when IC_JBC =>                -- JBC bit, rel
            if state=FETCH then
              ...
              s_adr_mux <= "01000";      -- bit adress = rom_data_i
              s_help_en <= "00001";      -- s_help = rom_data_i
              s_pc_inc_en <= "0001";    -- increment program-counter
              s_nextstate <= EXEC2;
            elsif state=EXEC2 then
              s_adr_mux <= "01010";       -- s_adr = s_help
          ...
          when IC_MOV_RR_D =>           -- MOV Rr, direct
            if state=FETCH then
              s_help_en <= "00100";      -- save Rr-adress in help
              s_pc_inc_en <= "0001";    -- increment program-counter
              s_nextstate <= EXEC1;
            elsif state=EXEC1 then
              s_adr_mux <= "01000";      -- address = rom_data_i
          ...
          when IC_MOV_D_DATA =>         -- MOV direct, #data
            if state=FETCH then
              ...
              s_help_en <= "00001";      -- save direct adress
              s_pc_inc_en <= "0001";    -- increment program-counter
              s_nextstate <= EXEC2;
            elsif state=EXEC2 then
              s_adr_mux <= "01010";      -- address = help
          ...
          when IC_ORL_D_DATA =>         -- ORL direct, DATA
            if state=FETCH then
              ...
              s_help_en <= "00001";      -- save rom_data_i
              s_adr_mux <= "01000";
              s_pc_inc_en <= "0001";    -- increment program-counter
              s_nextstate <= EXEC2;
            elsif state=EXEC2 then
          ...
          when IC_ORL_C_BIT =>          -- ORL C, bit
            if state=FETCH then
              ...
              s_regs_wr_en <= "0110";    -- write one bit (autmatically CY)
              s_pc_inc_en <= "0001";    -- increment program-counter
              s_nextstate <= FETCH;
            end if;
          ...
          when IC_POP =>                -- POP direct
            if state=FETCH then
              s_adr_mux <= "00101";      -- address = SP
              s_pc_inc_en <= "0001";    -- increment program-counter
              s_nextstate <= EXEC1;
            elsif state=EXEC1 then
              s_adr_mux <= "01000";      -- address = rom_data_i
              s_data_mux <= "0100";     -- data = s_reg_data
              s_regs_wr_en <= "0101";    -- write one byte and decrement SP
              s_pc_inc_en <= "0001";    -- increment program-counter
              s_nextstate <= FETCH;
            end if;
          ...
          when IC_PUSH =>               -- PUSH direct
            if state=FETCH then
              s_regs_wr_en <= "0001";    -- increment SP
              s_pc_inc_en <= "0001";    -- increment program-counter
              s_nextstate <= EXEC1;
            elsif state=EXEC1 then
              s_adr_mux <= "01000";      -- address = rom_data_i
          ...
          when IC_RET =>                -- RET
            if state=FETCH then
              s_adr_mux <= "00101";      -- adress = SP
              s_regs_wr_en <= "0001";    -- decrement stackpointer
              s_nextstate <= EXEC1;
            elsif state=EXEC1 then
              s_help_en <= "00011";      -- save data for PC-high
              s_adr_mux <= "00101";      -- adress = SP
              s_nextstate <= EXEC2;
            elsif state=EXEC2 then
              s_regs_wr_en <= "0001";    -- decrement stackpointer
          ...
          when IC_XCH_A_RR =>           -- XCH A,Rr
            if state=FETCH then
              ...
              s_adr_mux <= "00110";      -- address = rr-address
              s_help_en <= "00011";      -- help = s_reg_data
              s_data_mux <= "0110";     -- data = ACC
              s_regs_wr_en <= "0100";    -- write ACC
              s_nextstate <= EXEC2;
            elsif state=EXEC2 then
          ...
          when IC_XCHD_A_ATRI =>        -- XCHD A,@Ri
            if state=FETCH then
              s_adr_mux <= "00111";      -- address = Ri-register
              s_help_en <= "01010";      -- s_help = acc
              s_nextstate <= EXEC1;
            elsif state=EXEC1 then
              s_adr_mux <= "00111";      -- address = Ri-register
              s_data_mux <= "1001";   -- data = s_help(7..4) & ram_data_i(3..0)
              s_regs_wr_en <= "0010";    -- write ACC
              s_nextstate <= EXEC2;
            elsif state=EXEC2 then
              s_adr_mux <= "00111";    -- address = Ri-register
              s_data_mux <= "1010";     -- data = ram_data_i(7..4) & s_help(3..0)
              s_regs_wr_en <= "1000";    -- write one byte to RAM
              s_pc_inc_en <= "0001";  -- increment program-counter
              s_nextstate <= FETCH;
            end if;
